...
@router.message(UserCommand('warn', description=get_string('command_description/warn'),
							opportunity=CommandOpportunity.warn))
async def command(message: Message, user: User, state: FSMContext):
	await message.delete()
	msg = await bot.send_message(user.id, get_string('warn/command/user'))
...
async def user_state(message: Message, user: User, state: FSMContext):
	...
	try:
		...
		await bot.send_message(user.id, get_string('warn/command/error/value'))
		return

	target = session.scalar(select(User).where(User.fake_id == target_id))
	if not target:
		await bot.send_message(user.id, get_string('warn/command/error/user'))
		return

	msg = await bot.send_message(user.id, get_string('warn/command/type'),
								 reply_markup=ReplyKeyboardMarkup(
									 keyboard=[[KeyboardButton(text=section['text'])]
											   for section in get_string('rules/sections')]))
...
async def type_state(message: Message, user: User, state: FSMContext):
	...
	if not index:
		await bot.send_message(sender.id, get_string('warn/command/error/type'), reply_markup=hide_markup)
		return
	...
	section = get_string('rules/sections')[index]
	...
	for user in session.scalars(select(User).where(User.id != target.id)).all():
		tasks += [bot.send_message(user.id, get_string('warn/broadcast').format_map(format_map))]
	tasks += [bot.send_message(target.id, get_string('warn/user').format_map(format_map))]
